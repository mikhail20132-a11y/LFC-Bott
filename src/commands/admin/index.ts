...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName("setup").setDescription("Auto-configure the LFC bot for this server (Founder only)")
    )
    .addSubcommand((sub) =>
    ...
    .addSubcommand((sub) =>
      sub.setName("suspend").setDescription("Suspend a user from the league (Management only)")
    ...
    .addSubcommand((sub) =>
      sub.setName("blacklist").setDescription("Blacklist a user from the league (Founder only)")
    ...
    .addSubcommandGroup((group) =>
      group.setName("season").setDescription("Season management")
        .addSubcommand((sub) => sub.setName("start").setDescription("Start a new season (Management only)")
          .addStringOption((opt) => opt.setName("name").setDescription("Season name").setRequired(true)))
        .addSubcommand((sub) => sub.setName("end").setDescription("End the current active season (Management only)"))
    ),
  ...
  async execute(interaction: CommandInteraction) {
    ...
    const handlers: Record<string, (i: CommandInteraction) => Promise<void>> = {
      setup: handleSetup, warn: handleWarn, suspend: handleSuspend, blacklist: handleBlacklist,
    };
...
async function handleSetup(interaction: CommandInteraction): Promise<void> {
  ...
  try {
    // 1. Create roles
    const founderRole = await guild.roles.create({ name: "Founder", color: 0xFFD700, reason: "LFC Auto-Setup" });
...
async function handleWarn(interaction: CommandInteraction): Promise<void> {
  await interaction.deferReply();
  if (!hasRole(interaction.member as never, RoleType.Moderator) && !hasRole(interaction.member as never, RoleType.Management) && !hasRole(interaction.member as never, RoleType.Founder)) {
...
async function handleSuspend(interaction: CommandInteraction): Promise<void> {
  await interaction.deferReply();
  if (!hasRole(interaction.member as never, RoleType.Management) && !hasRole(interaction.member as never, RoleType.Founder)) {
...
async function handleBlacklist(interaction: CommandInteraction): Promise<void> {
  ...
  if (!hasRole(interaction.member as never, RoleType.Founder)) {
    await interaction.editReply({ embeds: [createErrorEmbed("❌ No Permission", "Founder only.")] });
    return;
  }
  const user = interaction.options.getUser("user", true);
  const reason = interaction.options.getString("reason", true);
